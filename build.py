...
def nav_html(cur):
    ...
    for href, label, kids in NAV:
        if kids:
            chev = ('<svg class="chev" width="9" height="6" viewBox="0 0 9 6" fill="none" aria-hidden="true">'
                    '<path d="M1 1L4.5 4.5L8 1" stroke="currentColor" stroke-width="1.3" '
                    'stroke-linecap="round" stroke-linejoin="round"/></svg>')
            out.append('<div class="drop"><button aria-expanded="false">%s%s</button><div class="drop-m">%s</div></div>'
                       % (label, chev, "".join('<a href="%s">%s</a>' % k for k in kids)))
        else:
...
FAQ_ITEMS = [
 ...
 ("כמה טיפולים אני צריכה?",
  "תלוי באזור, בסוג השיער ובעור. סדרה סטנדרטית היא בדרך כלל בסביבות 10 טיפולים, במרווחים של 4-6 שבועות. בטסט אני בודקת את זקיק השערה ונותנת לך מספר ריאלי, ולא מספר שנשמע טוב במכירה."),
 ("האם הטיפול כואב?",
...
def frame_img(src, alt, label, note="", ratio="16/11", doc=False):
    """doc=True, צילום תיעודי (לפני/אחרי). מוצג במלואו, בלי חיתוך."""
    if os.path.exists(src):
        if doc:
            return ('<div class="frame doc" style="border-radius:var(--r-lg);overflow:hidden;margin-top:28px">'
                    '<img src="%s" alt="%s" loading="lazy"></div>' % (src, alt))
        return ('<div class="frame" style="aspect-ratio:%s;border-radius:var(--r-lg);overflow:hidden;position:relative;margin-top:28px">'
                '<img src="%s" alt="%s" loading="lazy" style="width:100%%;height:100%%;object-fit:cover"></div>' % (ratio, src, alt))
...
CTX = {
  ...
  "fig_plasma":   frame_img("assets/service-plasma.jpg", "לפני ואחרי: הסרת נימים בפנים", "טיפול פלזמה", "assets/plasma-treatment.jpg", doc=True),
